...
async function showRes() {
  ...
  if (!viewMode.value) {
    if (questionRepo.find(questionId).score === questionRepo.find(questionId).maxScore) {
      questionRepo.save({
        id: questionId,
...
<template>
  <div class="question-control">
    <div class="controls-container">
      <button v-if="!editMode" class="button" @click="previous()">
      ...
      <div class="slider-container">
        ...
        <div class="slider-index">{{ 1 + Number(index) }}/{{ questions.length }}</div>
      </div>
      <button v-if="!editMode" class="button" @click="next()">
        <span class="but-text">Next</span>
      </button>
    </div>
    <div v-if="!viewMode && !editMode" class="data-container">
      <button
