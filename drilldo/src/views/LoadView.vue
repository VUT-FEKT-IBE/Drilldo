...
<template>
  <div class="loader-container">
    ...
    <div class="but-par">
      <p v-if="questionRepo.all().length === 0">First you need to upload your questions:</p>
      <UploadJsonComponent
        v-if="questionRepo.all().length === 0"
        button-text="Load Questions"
        @data="getData" />
    </div>
    <div v-if="questionRepo.all().length >= 1" class="but-container">
      <div v-if="!store.statsLoaded" class="but-par">
        <p class="stats text">
          (Optional) You can also upload statistics from a prior session to allow question shuffle
          to prefer questions where you make mistakes.
        </p>
        <UploadJsonComponent button-text="Load statistics" @data="getStats" />
      </div>
    </div>
    <div v-if="questionRepo.all().length >= 1" class="navigator">
    ...
    <div v-if="questionRepo.all().length >= 1" class="but-par offset">
      <p class="reset text">If you want to clear all questions, click this button:</p>
      <button
        class="reset button"
        @click="
          () => {
